...
for (i in 1:nrow(ring26)) {
  ...
  pos_newML <- ifelse(sign_newML == 1, positions[2]-1, positions[2]+1)
  if (positions[3] > 2) {
...
rescaled <- Rescale(trim_expand, 23, 22, 23)
rescaled_out <- tibble(AntPos = rescaled$APint, 
                         MedLat = rescaled$MLint, 
                         VenDor = rescaled$VDint, type = "straight_medwall") %>%
  unique()
OBplot(rescaled_out)
#saveRDS(rescaled_out, "~/Desktop/obmap/r_analysis/mri_to_R/output/210310_cubegrid_rescaled_outeronly.RDS")
```
