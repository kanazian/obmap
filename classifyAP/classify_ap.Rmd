...
#load data
Prop.OR <- readRDS("~/Desktop/obmap/r_analysis/classifyAP/input/PropOR.RData")
all_positions <- read_csv("~/Desktop/obmap/r_analysis/classifyAP/alldimwavg_diffe_DV_info.csv")
OR_aln <- read.alignment(file = "~/Desktop/obmap/r_analysis/classifyAP/input/kz_gen25_mafftout_210307.aln", format = "clustal")
OR_aln$nam <- as.character(lapply(strsplit(OR_aln$nam, split="/"), "[", 1))
tgrantham <- read.csv("~/Desktop/obmap/r_analysis/classifyAP/input/grantham.csv", header = T)
AAProp <- readRDS("~/Desktop/obmap/r_analysis/classifyAP/input/AAprop.RData")
...
filter_positions <- all_positions %>% 
  filter(filter == "Pass") %>%
  filter(class == 2) %>% 
  select(gene:aphm_wavg, vd_wavg, oe_region, tz_vd) %>%
  ...
  mutate(rawrank_3d = min_rank(ap3d_wavg),
         rawrank_hm = min_rank(aphm_wavg)) %>%
  group_by(oe_bestguess) %>%
  mutate(vdrank_3d = min_rank(ap3d_wavg),
         vdrank_hm = min_rank(aphm_wavg)) %>%
  ungroup()

dorsal_n <- nrow(filter_positions %>% filter(oe_bestguess == "Dorsal"))
ventral_n <- nrow(filter_positions %>% filter(oe_bestguess == "Ventral"))
splitp <- 0.4
# AP groups will consist of 40% of DV group size, with middle 20% not used

# make AP groups
# 3d indicates sort made from mice used in 3D model (m10,13,15)
dor_3d_ant <- filter_positions %>%
  filter(oe_bestguess == "Dorsal") %>%
  filter(vdrank_3d > (dorsal_n * (1-splitp))) %>%
  arrange(vdrank_3d) %>%
  pull(gene)

dor_3d_post <- filter_positions %>%
  filter(oe_bestguess == "Dorsal") %>%
  filter(vdrank_3d <= (dorsal_n * splitp)) %>%
  arrange(vdrank_3d) %>%
  pull(gene)

ven_3d_ant <- filter_positions %>%
  filter(oe_bestguess == "Ventral") %>%
  filter(vdrank_3d > (ventral_n * (1-splitp))) %>%
  arrange(vdrank_3d) %>%
  pull(gene)

ven_3d_post <- filter_positions %>%
  filter(oe_bestguess == "Dorsal") %>%
  filter(vdrank_3d <= (ventral_n * splitp)) %>%
  arrange(vdrank_3d) %>%
  pull(gene)
...
dor_hm_ant <- filter_positions %>%
  filter(oe_bestguess == "Dorsal") %>%
  filter(vdrank_hm > (dorsal_n * (1-splitp))) %>%
  arrange(vdrank_hm) %>%
  pull(gene)

dor_hm_post <- filter_positions %>%
  filter(oe_bestguess == "Dorsal") %>%
  filter(vdrank_hm <= (dorsal_n * splitp)) %>%
  arrange(vdrank_hm) %>%
  pull(gene)

ven_hm_ant <- filter_positions %>%
  filter(oe_bestguess == "Ventral") %>%
  filter(vdrank_hm > (ventral_n * (1-splitp))) %>%
  arrange(vdrank_hm) %>%
  pull(gene)

ven_hm_post <- filter_positions %>%
  filter(oe_bestguess == "Dorsal") %>%
  filter(vdrank_hm <= (ventral_n * splitp)) %>%
  arrange(vdrank_hm) %>%
  pull(gene)
```
...
saveRDS(kzmtx, "~/Desktop/obmap/r_analysis/classifyAP/output/mtx1500_newbasal_kzaln_210307.RDS")
```

##Find significant sites
```{r, eval=F}
#load made mtx
#kzmtx <- readRDS("~/Desktop/obmap/r_analysis/classifyAP/output/mtx1500_dorsal_ap3d_kzaln_210307.RDS")
...
print_aa(olfr539aln, which(sigs_fdr < 0.05), surround = 1)
