...
```{r}
ListDorML(c("Olfr881", "Olfr1377"), by = 50, out = "point", title = "mKate2-ORs")
ListDorML(c("Olfr881", "Olfr1377"), by = 50, title = "mKate2-ORs")
```


#ZZZworking
# updating DorsalML function to better predict unusual zone ORs and ORs close to symline
if unusual: find single best cluster regardless of symline? or perhaps target unusual zone
if near symline: find single best cluster? perhaps enlarge?
if dorsal: DorsalML
if ventral: DorsalML
After 1st round of postions, determine how many voxels are multimapped, kick out 
from 210404 vox4B, potentially 488 gloms to redistribute if max 2 glom per voxel
Allow medial glom to have slightly more variable DV position

it may be faster to split ranked into Med and Lat dfs, and then run Cluster on each DF
Maybe even splitting into MedDor, MedVen, LatDor, LatVen

PredictList called PredictGloms on list of genes
PredictGloms calls Cluster on a gene and picks appropriate candidates based on DV and symline position
```{r}
ranked <- readRDS("~/Desktop/obmap/r_analysis/data/3dimOB/model/210404_vox4B_rk_dtvt_OConly.RDS") %>% 
  rowwise() %>% 
  mutate(symside = ifelse(MedLat >= symline$mlvals[which(symline$apvals == AntPos)] - 2, "Lateral", "Medial")) %>%
  ungroup()
...
ranked_MD <- ranked_Msym %>% filter(VenDor >= 10)
ranked_MV <- ranked_Msym %>% filter(VenDor < 14)
ranked_LD <- ranked_Lsym %>% filter(VenDor >= 12)
ranked_LV <- ranked_Lsym %>% filter(VenDor < 12)
ranked_unu <- ranked %>% filter(between(MedLat, 4, 11)) %>%
  filter(between(VenDor, 5, 10))

nrow(ranked) == (nrow(ranked_MD) + nrow(ranked_MV) + nrow(ranked_LD) + nrow(ranked_LV))

tic()
Cluster("Olfr1377", topX = 75, df = ranked, out = "data")
toc()

tic()
Cluster("Olfr1377", topX = 20, df = ranked_LD, out = "plot")
Cluster("Olfr1377", topX = 20, df = ranked_MD, out = "plot")
toc()

DVinfo <- info %>% 
  select(olfrname:tz_vd, vd_mp) %>%
  rowwise() %>%
  ...
  select(olfrname, DVdiscord, DVcertainty, vd_mp)

DVinfo %>% select(DVdiscord, vd_mp) %>% filter(!is.na(DVdiscord)) %>% mutate(vd_rank = min_rank(vd_mp)) %>% ggplot() + geom_violin(aes(DVdiscord, vd_rank))

#ZZZworking
PredGloms <- function(olfr, topMin = 20, topBy = 10, 
                      minSize = 3, clustOut = 1, 
                      symconstrain = F, verbose = F,
                      out = "plot", title = NA) {
  ...
  while (clustFound < clustOut) {
    if (!str_detect(olfr, "Olfr")) {
      df_lat <- Cluster(olfr, df = ranked_LD, 
                        ...
                        out = "best") %>% 
        mutate(side = "Lat", step = topStep)
      if (symconstrain == T) {
        mostAnt <- df_lat %>%
          pull(AntPos) %>%
          min()
        ranked_MDsym <- ranked_MD %>%
          filter(AntPos >= (mostAnt - 1))
        df_med <- Cluster(olfr, df = ranked_MDsym,
                          topX = topStep, minClustSize = minSize,
                          out = "best") %>% 
          mutate(side = "Med", step = topStep)
      ...
      if (DVpos[1] == "Unusual") {
        ...
        if (symconstrain == T) {
          mostAnt <- df_lat %>%
            pull(AntPos) %>%
            min()
          ranked_MVsym <- ranked_MV %>%
            filter(AntPos >= (mostAnt - 1))
          df_med <- Cluster(olfr, df = ranked_MVsym,
                            topX = topStep, minClustSize = minSize,
                            out = "best") %>% 
            mutate(side = "Med", step = topStep)
          df_all <- bind_rows(df_lat, df_med) %>%
...
PredList <- function(x, by = 10, out = "plot", title = NA) {
  #use a list to build a df of unknown size instead of bind_row each iteration
  list_out <- vector("list", length = length(x))
  for (i in 1:length(x)) {
    list_out[[i]] <- PredGloms(x[i], topBy = by, symconstrain = T, verbose = T, out = "best")
  } #endfori
...
df_in <- ranked_unu %>% 
  filter(olfrname == "Olfr155") %>% 
  mutate(rankofrank = min_rank(desc(p50))) %>% 
  filter(rankofrank <= 25) %>% 
  arrange(desc(p50))
...
for (k in 1:ncol(dmatrix)) {
  ...
  while (neighlenchange != 0) {
    ...
    neighinit <- neighlen
    print(paste("k:", k))
    print(paste("neighlen:", length(neigh)))
  } #endwhile
  cluster_list[neigh] <- k
} #endfork
