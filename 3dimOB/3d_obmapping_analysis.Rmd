...
#make tib of most ventral and most dorsal outliers for each binval, check if p50 values are outliers for groups 
for (i in min(filter_preds$tzbins):max(filter_preds$tzbins)) {
  filtered_lat <- filter_preds %>%
    filter(p50 == clustmaxp) %>%
    filter(side == "Lateral") %>%
    filter(tzbins == i) %>%
    mutate(vdrank = rank(desc(VenDor), ties.method = "random"),
           checkdor = ifelse(vdrank <= 10, 100, 0),
           checkven = ifelse(vdrank > (max(vdrank) - 10), 10, 0),
           checkmid = ifelse(vdrank == median(vdrank), 1, 0)) %>%
    rowwise() %>%
    mutate(pack = checkdor + checkven + checkmid) %>%
    ungroup() %>%
    select(olfrname, side, VenDor, tzsimple, tzbins, p50, vdrank, pack) %>%
    filter(pack > 0)
  ...
  filtered_both <- bind_rows(filtered_lat, filtered_med)
  
  if (i == min(filter_preds$tzbins)) {
    outliers <- filtered_both
  } else if (i == max(filter_preds$tzbins)) {
    outliers <- bind_rows(outliers, filtered_both) %>%
      mutate(type = ifelse(pack == 100, "dor",
                           ifelse(pack == 10, "ven",
                                  ifelse(pack == 1, "mid", "woops"))))
  } else if (i == 3) {
    #seems that tzbin 3 holds only 10 OR points total for both med and lat sides
    filtered_lat <- filter_preds %>%
    filter(p50 == clustmaxp) %>%
    filter(side == "Lateral") %>%
    filter(tzbins == i) %>%
    mutate(vdrank = rank(desc(VenDor), ties.method = "random"),
           checkdor = ifelse(vdrank <= 3, 100, 0),
           checkven = ifelse(vdrank > (max(vdrank) - 3), 10, 0),
           checkmid = ifelse(between(vdrank, median(vdrank) - 1.9, median(vdrank) + 1.9), 1, 0)) %>%
    rowwise() %>%
    mutate(pack = checkdor + checkven + checkmid) %>%
    ungroup() %>%
    select(olfrname, side, VenDor, tzsimple, tzbins, p50, vdrank, pack) %>%
...
outliers %>% 
  filter(side == "Lateral") %>% 
  group_by(tzbins, type) %>% 
  summarise(avgp50 = mean(p50)) %>% 
  ggplot() + 
  geom_point(aes(type, avgp50)) +
  facet_wrap(~ tzbins)
