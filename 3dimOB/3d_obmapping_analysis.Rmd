...
Analyze_DVoutliers <- function(df, index, chooseOut = "Lat") {
  if(str_detect(tolower(chooseOut), "lat")) {
    ...
    type_stats <- df %>% 
      ...
      select(type_name, group_p50, type_hampel_lo)
    ...
    mid_stats <- df %>% 
      ...
      summarise(mid_p50 = mean(p50), 
                ...
                mid_1sd = mean(p50) - sd(p50)) %>% 
      ungroup() %>% rowwise() %>%
      mutate(mid_name = paste0(VenDor, side)) %>% 
      select(mid_name, mid_p50, mid_min, mid_1mad, mid_1sd)
    ...
    df_out <- df %>% 
      ...
      mutate(out_below_mid = ifelse(type != "mid", ifelse(p50 < mid_p50, T, F), F),
             out_below_midmin = ifelse(out_below_mid == T, ifelse(p50 < mid_min, T, F), F),
             out_below_1mad = ifelse(out_below_mid == T, ifelse(p50 < mid_1mad, T, F), F),
             out_below_1sd = ifelse(out_below_mid == T, ifelse(p50 < mid_1sd, T, F), F)) %>%
      select(-type_name, -mid_name, -checkhi, -checkmid, -checklo, -barhi, -barlo, -barmid, -pack)
    return(df_out)
  } #endif
} #endfunc
