...
```{r}
#feature info
info <- read_csv("~/Desktop/obmap/r_analysis/3dimOB/input/info_201207.csv", 
                 col_names = TRUE) %>% 
...
#blankOBcoords200922
blankdata <- readRDS("~/Desktop/obmap/r_analysis/mri_to_R/output/210310_cubegrid_rescaled_outeronly.RDS") %>% select(-type)
dvml_blank <- blankdata %>%
  ...
  rowwise() %>%
  mutate(symdim = ifelse(MedLat == round(symline$mlvals[which(symline$apvals == AntPos)]),
                         "SymLine", dim)) %>%
  ungroup()
...
filter_olfrs <- readRDS("~/Desktop/obmap/r_analysis/heatmaps/output/filterORs.RDS")

#load 3d model predictions (clusters) from ListDorML function with non-OLfr genes expected to be Dorsal
filter_preds <- readRDS("~/Desktop/obmap/r_analysis/3dimOB/output/e_allDors_by50.RDS")
...
```{r}
mom_pick <- read_csv("~/Desktop/Mombcomp_picalign_coords.csv") %>% filter(model == 2018)
...
for (i in 1:nrow(mom_pick)) {
  ...
  best_pick <- c(mean(c(mom_pick$minAP[i], mom_pick$maxAP[i])),
                 mean(c(mom_pick$minVD[i], mom_pick$maxVD[i])),
                 mean(c(mom_pick$minML[i], mom_pick$maxML[i])))
  if (nrow(mom_point > 0)) {
...
#saveRDS(mom_score, "~/Desktop/mombcomp_d2.RDS")

#are there systematic biases in best point distance?
ggplot(mom_score, aes(bestAPdist)) + geom_dotplot() + geom_density()
ggplot(mom_score, aes(bestMLdist)) + geom_dotplot() + geom_density()
ggplot(mom_score, aes(bestVDdist)) + geom_dotplot() + geom_density()
```
